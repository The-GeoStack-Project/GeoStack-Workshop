...
sure this connection is assigned to another variable. 
...
trail_connection = PyMongo(app, uri="mongodb://localhost:27017/Trail_Database") #TODO
...
def get_one_trail(id):
    ...
    query_result = trail_connection.db.trail.find({"_id": ObjectId(id)}) #TODO 
...
def get_all_signals_amount(id,amount):
    ...
    query_result_id = trail_connection.db.signal.find({"trail": ObjectId(id)})[:int(amount)] #TODO
...
def get_all_trackers():
    ...
    query_result = crane_connection.db.tracker.find() #TODO
...
def get_all_transmissions_by_id(id):
    ...
    query_result_id = crane_connection.db.transmission.find({"tracker": ObjectId(id)}) #TODO
